...
void EBS_CheckUP_Routine (void)
{
    ...
    if ( ucCheck == 6 )
    {
        if ( ( ANALOG_GetVoltage(2) >= NPRES_min ) & ( ANALOG_GetVoltage(2) >= NPRES_min ) )
        {
            ucCheck = 7;
        }
    }
    ...
    if ( ucCheck == 7 )
    {
        if ( ( ANALOG_GetVoltage(0) <= HDRPRES_min ) & ( ANALOG_GetVoltage(0) <= HDRPRES_min ) )
        {
            ucCheck = 8;
        }
    }
    ...
    if ( ucCheck == 16 )
    {
        CANWriteMessage ( 0x362, 8, ucASBState, ucASRequesState, 0, 0, 0, 0, 0, 0 );
    }
